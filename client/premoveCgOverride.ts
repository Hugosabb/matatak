...
function forward(color: cg.Color, m: Mobility): Mobility {
  return (x1, y1, x2, y2) => {
    return (color === 'white' ? y2 > y1 : y2 < y1) && m(x1, y1, x2, y2);
  };
}
...
// matatak pieces
const meerkat = (c: cg.Color) => or(forward(c, wazir), forward(c, ferz));
const cat = (c: cg.Color) => forward(c, kingNoCastling);
const leopard = and(queen, distance(2));
const tiger = queen;
const tortoise = (c: cg.Color) => forward(c, wazir);
const warthog = (c: cg.Color) => forward(c, and(rook, distance(2)));
const panda = wazir;
const bear = and(rook, distance(3));
const rhino = rook;
const lemur = (c: cg.Color) => forward(c, ferz);
const gibbon = (c: cg.Color) => forward(c, and(bishop, distance(2)));
const chimpanzee = ferz;
const orangutan = and(bishop, distance(3));
const gorilla = bishop;
const rabbit = (c: cg.Color) => forward(c, (x1: number, y1: number, x2: number, y2: number) => {
  const xd = diff(x1, x2);
  const yd = diff(y1, y2);
  return (xd === 0 && yd === 2) || (xd === 2 && yd === 0);
});
const gazelle = (c: cg.Color) => forward(c, knight);
const giraffe = (x1: number, y1: number, x2: number, y2: number) => {
  const xd = diff(x1, x2);
  const yd = diff(y1, y2);
  return (xd === 1 && yd === 3) || (xd === 3 && yd === 1);
};
const kangaroo = (x1: number, y1: number, x2: number, y2: number) => {
  const xd = Math.abs(x1 - x2);
  const yd = Math.abs(y1 - y2);
  if (xd === 0 || yd === 0) return false;
  return xd / yd === 2 || yd / xd === 2;
};
  ...
  switch (variant) {
    ...
    case 'matatak':
      return (boardState, key) => {
        ...
        switch (role) {
          case 'a-piece': return meerkat(color);
          case 'b-piece': return cat(color);
          case 'c-piece': return leopard;
          case 'd-piece': return tiger;
          case 'e-piece': return tortoise(color);
          case 'f-piece': return warthog(color);
          case 'g-piece': return panda;
          case 'h-piece': return bear;
          case 'i-piece': return rhino;
          case 'j-piece': return lemur(color);
          case 'l-piece': return gibbon(color);
          case 'm-piece': return chimpanzee;
          case 'n-piece': return orangutan;
          case 'o-piece': return gorilla;
          case 'p-piece': return rabbit(color);
          case 'q-piece': return gazelle(color);
          case 'r-piece': return giraffe;
          case 's-piece': return knight;
          case 't-piece': return kangaroo;
          case 'w-piece': return or(bishop, kingNoCastling);
          case 'x-piece': return or(rook, kingNoCastling);
          case 'y-piece': return or(rook, knight);
          case 'z-piece': return or(wazir, knight);
          case 'k-piece': return kingNoCastling;
          default:
            return noMove;
        }
      };
